...
#define Default_OverLook                   @"忽略"
#define Default_ArrowDownLoad              @"前往下载"
...
//stretch image
#define StrechImage(image, top, left, bottom, right) (CurrentSystemVersion_Double < 6.0 ? [image stretchableImageWithLeftCapWidth:left topCapHeight:top] : [image resizableImageWithCapInsets:UIEdgeInsetsMake(top, left, bottom, right) resizingMode:UIImageResizingModeStretch])

//泛型转换成NSString
#define StringFromId(idType)  [NSString stringWithFormat:@"%@",idType]

//整形转换成NSString
#define StringFromInt(idType)  [NSString stringWithFormat:@"%d",idType]
#define StringFromFloat(idType)  [NSString stringWithFormat:@"%f",idType]
//NSNull 的处理
